...
export default function AdminReports() {
    ...
    const { data: bookings = [] } = useQuery({
        ...
        queryFn: async () => { const { data } = await api.get('/admin/bookings'); return Array.isArray(data) ? data : []; },
    });
    ...
    return (
        <div className="p-6">
            <div className="flex items-center gap-3 mb-8">
                ...
                <div><h1 className="text-2xl font-bold text-white">{t('admin_reports')}</h1><p className="text-zinc-400 text-sm">{t('booking_revenue_reports')}</p></div>
            </div>

            <div className="bg-[#1F222A] border border-[#35383F] rounded-2xl p-6 mb-6">
                <h2 className="font-bold text-white mb-4">{t('filter_date_range')}</h2>
                <div className="flex flex-wrap gap-4 items-end">
                    <div>
                        <label className="block text-sm text-zinc-400 mb-1.5">{t('from')}</label>
                    ...
                    <div>
                        <label className="block text-sm text-zinc-400 mb-1.5">{t('to')}</label>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
                {[
                    { label: t('revenue'), value: `$${revenue.toFixed(2)}`, color: 'text-emerald-400' },
                    { label: t('completed'), value: completed, color: 'text-blue-400' },
                    { label: t('cancelled'), value: cancelled, color: 'text-red-400' },
                ].map((s, i) => (
            ...
            <div className="bg-[#1F222A] border border-[#35383F] rounded-2xl overflow-hidden">
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        {filtered.length === 0 && <tr><td colSpan={4} className="px-6 py-12 text-center text-zinc-500">{t('no_bookings_range')}</td></tr>}
                    </tbody>
                </table>
            </div>
        </div>
