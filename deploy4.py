#!/usr/bin/env python3
"""Deploy barber-shop-app - Phase 4: Create MySQL tables and seed data"""
import paramiko
import time
...
# Create all tables via raw SQL since drizzle-kit push needs TypeScript compilation
print("[1] Creating MySQL tables...")
...
# Write SQL to a file and execute via mysql CLI
run(client, f"""cat > /tmp/create_tables.sql << 'SQLEOF'
{create_tables_sql}
SQLEOF""", show=False)

print("Running SQL...")
out, err = run(client, f"mysql -u u492425110_barber -p'K68\$~Tf6=' u492425110_barber < /tmp/create_tables.sql 2>&1")

# Verify tables exist
print("\n[2] Verifying tables...")
out, _ = run(client, f"mysql -u u492425110_barber -p'K68\$~Tf6=' u492425110_barber -e 'SHOW TABLES;' 2>&1")

# Now restart the app so the seed runs
print("\n[3] Restarting app...")
