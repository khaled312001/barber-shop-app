...
export default function LandingPages() {
    ...
    const { data: salons = [], isLoading } = useQuery<Salon[]>({
        ...
        queryFn: async () => { const { data } = await api.get('/admin/landing-pages'); return Array.isArray(data) ? data : []; },
    });
    ...
    return (
        <div className="p-6">
            <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-white">{t('landing_pages')}</h1>
                    <p className="text-zinc-400 text-sm">{t('manage_landing')}</p>
                </div>
                <div className="ml-auto flex items-center gap-2 bg-[#1F222A] border border-[#35383F] rounded-xl px-4 py-2">
                    <Globe size={14} className="text-green-400" />
                    <span className="text-xs text-zinc-300">
                        {salons.filter(s => s.landingEnabled && s.landingSlug).length} {t('pages_live')}
                    </span>
                </div>
            </div>

            {isLoading ? (
                <div className="flex items-center justify-center h-48 text-zinc-500">{t('loading')}</div>
            ) : salons.length === 0 ? (
                <div className="bg-[#1F222A] border border-[#35383F] rounded-2xl p-12 text-center">
                    ...
                    <h2 className="text-xl font-bold text-white mb-2">{t('no_salons_admin')}</h2>
                    <p className="text-zinc-400">{t('create_salons_hint')}</p>
                </div>
            ) : (
                            ...
                            <div key={salon.id} className="bg-[#1F222A] border border-[#35383F] rounded-2xl overflow-hidden">
                                ...
                                <div className="flex items-center gap-4 px-5 py-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-zinc-500 text-xs truncate">{salon.address || t('no_data')}</p>
                                    </div>
                                    ...
                                    {salon.landingEnabled && salon.landingSlug ? (
                                        ...
                                        <div className="hidden sm:flex items-center gap-1.5 bg-zinc-800 text-zinc-500 text-xs px-3 py-1 rounded-full border border-zinc-700 min-w-0 max-w-[180px]">
                                            ...
                                            <span className="truncate">{salon.landingSlug ? t('disabled') : t('no_slug')}</span>
                                        </div>
                                    )}
                                ...
                                {isExpanded && (
                                    <div className="border-t border-[#35383F] px-5 py-5 space-y-5">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                            ...
                                            <div>
                                                <label className="text-xs text-zinc-400 uppercase font-semibold tracking-wider flex items-center gap-1.5 mb-2">
                                                    <BookOpen size={12} />{t('booking_url')}
                                                </label>
                                                <input
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                            <div>
                                                ...
                                                <div className="flex gap-2">
                                                    {(['dark', 'light'] as const).map(themeVal => (
                                                        <button
                                                            key={themeVal}
                                                            onClick={() => setField(salon.id, 'landingTheme', themeVal)}
                                                            className={`flex-1 py-2 rounded-xl text-sm font-semibold border transition-all capitalize ${(edit.landingTheme || 'dark') === themeVal
                                                                ? 'bg-[#F4A460] text-black border-[#F4A460]'
                                                                : 'bg-[#181A20] text-zinc-400 border-[#35383F] hover:border-zinc-500'
                                                                }`}
                                                        >
                                                            {themeVal === 'dark' ? `🌙 ${t('dark_theme')}` : `☀️ ${t('light_theme')}`}
                                                        </button>
                                                    ))}
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                <label className="text-xs text-zinc-400 uppercase font-semibold tracking-wider flex items-center gap-1.5 mb-2">
                                                    <Palette size={12} />{t('accent_color')}
                                                </label>
                                            ...
                                            {liveUrl && (
                                                <>
                                                    ...
                                                    >
                                                        ...
                                                        {copied === salon.landingSlug ? t('copied') : t('copy_link')}
                                                    </button>
                                                </>
                                            )}
