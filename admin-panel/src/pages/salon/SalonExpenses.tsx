...
export default function SalonExpenses() {
    ...
    const { data: expenses = [], isLoading } = useQuery({
        ...
        queryFn: async () => { const { data } = await api.get('/salon/expenses'); return Array.isArray(data) ? data : []; },
    });
        ...
        <div className="p-6">
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <div><h1 className="text-2xl font-bold text-white">{t('salon_expenses')}</h1><p className="text-zinc-400 text-sm">{t('track_costs')}</p></div>
                </div>
            ...
            <div className="bg-[#1F222A] border border-[#35383F] rounded-2xl p-5 inline-block mb-6">
                <p className="text-zinc-400 text-sm mb-1">{t('total_expenses')}</p>
                ...
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
                    <div className="bg-[#1F222A] border border-[#35383F] rounded-2xl p-6 w-full max-w-md">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-lg font-bold text-white">{t('add_expense')}</h2>
                            <button onClick={() => setShowForm(false)} className="text-zinc-400 hover:text-white"><X size={20} /></button>
                        </div>
                        <div className="space-y-4">
                            {[
                                { label: t('description'), key: 'description', type: 'text', placeholder: 'Supplies' },
                                { label: t('amount'), key: 'amount', type: 'number', placeholder: '100' },
                                { label: t('category'), key: 'category', type: 'text', placeholder: `${t('rent')}, ${t('utilities')}...` },
                                { label: t('date'), key: 'date', type: 'date', placeholder: '' },
                            ].map(f => (
                                <div key={f.key}>
                        ...
                        {isLoading ? (
                            ...
                            <tr key={e.id} className="border-b border-[#35383F]/50 hover:bg-white/5 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <button onClick={() => { if (confirm(t('delete_expense_confirm'))) remove.mutate(e.id); }} className="p-1.5 text-red-400 hover:text-red-300"><Trash2 size={15} /></button>
                                </td>
                            </tr>
                        ))}
                        {!isLoading && expenses.length === 0 && <tr><td colSpan={5} className="py-12 text-center text-zinc-500">{t('no_expenses_logged')}</td></tr>}
                    </tbody>
                </table>
            </div>
        </div>
