    ...
    return (
        <div className="p-6">
            <div className="flex items-center gap-3 mb-8">
                ...
                <div><h1 className="text-2xl font-bold text-white">Appointments</h1><p className="text-zinc-400 text-sm">Manage salon bookings</p></div>
            </div>
                            ...
                            <tr key={b.id} className="border-b border-[#35383F]/50 hover:bg-white/5 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex gap-1.5 flex-wrap">
                                        {b.status === 'pending' && (
                                            <button onClick={() => update.mutate({ id: b.id, status: 'confirmed' })} className="px-3 py-1 text-xs rounded-lg bg-blue-500/15 text-blue-400 hover:bg-blue-500/25">Confirm</button>
                                        )}
                                        {(b.status === 'pending' || b.status === 'confirmed') && (
                                            <button onClick={() => update.mutate({ id: b.id, status: 'completed' })} className="px-3 py-1 text-xs rounded-lg bg-emerald-500/15 text-emerald-400 hover:bg-emerald-500/25">Complete</button>
                                        )}
                                        {b.status !== 'cancelled' && b.status !== 'completed' && (
                                            <button onClick={() => update.mutate({ id: b.id, status: 'cancelled' })} className="px-3 py-1 text-xs rounded-lg bg-red-500/15 text-red-400 hover:bg-red-500/25">Cancel</button>
                                        )}
                                    </div>
                                </td>
                            </tr>
                        ))}
                        {!isLoading && bookings.length === 0 && <tr><td colSpan={6} className="py-12 text-center text-zinc-500">No appointments found</td></tr>}
                    </tbody>
                </table>
            </div>
        </div>
