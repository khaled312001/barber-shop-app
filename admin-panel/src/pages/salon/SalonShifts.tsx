...
export default function SalonShifts() {
    ...
    const { data: shifts = [], isLoading } = useQuery({
        ...
        queryFn: async () => { const { data } = await api.get('/salon/shifts'); return data; },
    });
    const { data: staff = [] } = useQuery({ queryKey: ['salon-staff'], queryFn: async () => { const { data } = await api.get('/salon/staff'); return data; } });
        ...
        <div className="p-6">
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <div><h1 className="text-2xl font-bold text-white">Shifts</h1><p className="text-zinc-400 text-sm">Manage staff work schedules</p></div>
                </div>
                ...
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
                    <div className="bg-[#1F222A] border border-[#35383F] rounded-2xl p-6 w-full max-w-md">
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm text-zinc-400 mb-1.5">Staff Member (optional)</label>
                            ...
                            <div>
                                <label className="block text-sm text-zinc-400 mb-1.5">Day</label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm text-zinc-400 mb-1.5">Start</label>
                                ...
                                <div>
                                    <label className="block text-sm text-zinc-400 mb-1.5">End</label>
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="border-b border-[#35383F] text-zinc-400">
                            ...
                            <th className="text-left px-6 py-4 font-medium">Staff</th>
                    ...
                    <tbody>
                        {isLoading ? (
                            ...
                            <tr key={s.id} className="border-b border-[#35383F]/50 hover:bg-white/5 transition-colors">
                                ...
                                <td className="px-6 py-4 text-zinc-400">{staff.find((m: any) => m.id === s.staffId)?.name || 'All staff'}</td>
                                <td className="px-6 py-4">
                                    <button onClick={() => { if (confirm('Delete shift?')) remove.mutate(s.id); }} className="p-1.5 text-red-400 hover:text-red-300"><Trash2 size={15} /></button>
                                </td>
                            </tr>
                        ))}
                        {!isLoading && shifts.length === 0 && <tr><td colSpan={4} className="py-12 text-center text-zinc-500">No shifts defined</td></tr>}
                    </tbody>
                </table>
            </div>
        </div>
